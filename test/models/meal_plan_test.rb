...
describe MealPlan do
  describe "validity" do
    ...
    it "requires a start date" do
      _(meal_plan.errors[:start_date]).must_include "can't be blank"
    end
    it "requires a end date" do
      _(meal_plan.errors[:end_date]).must_include "can't be blank"
    end
    it "requires a user" do
      _(meal_plan.errors[:user]).must_include "can't be blank"
    end
  end
  ...
  describe "generating a weekly plan" do
    ...
    it "populates a meal for each day between the start date and the end date" do
      _(meal_plan.meals.size).must_equal 0

      meal_plan.build_meals

      _(meal_plan.meals.size).must_equal 7
    end

    it "builds valid meals" do
      meal_plan.build_meals

      _(meal_plan.meals.all?(&:valid?)).must_equal true
    end
